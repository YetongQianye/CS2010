...
//1.分配并初始化一个jpeg解压对象
	...
	struct jpeg_decompress_struct  dinfo;//声明一个解压对象
	struct jpeg_error_mgr jerr;//声明一个结构体jerr保存出错信息
...
//2.指定要解压缩的图像文件	
	...
	{
		perror("fopen error");
		jpeg_destroy_decompress(&dinfo);//销毁dinfo这个解压对象	
		return -1;
	}
	//把jpeg文件和解压对象向关联
...
//5. 调用jpeg_start_decompress()启动解压过程
	...
	*/
	...
	//dinfo.output_scanline , 表示的意思是,已经读取了多少行,由dinfo结构体自动维护
	int x,y;
	int i=0;
	...
	{
		y = dinfo.output_scanline; //已经读取到第几行了(y)
		//printf("y = %d\n",y);
		jpeg_read_scanlines(&dinfo,  //解压对象
							&buffer,//保存解压后数据的二级指针, 
							1 //读取多少行数据来解压
							);					
		//读取一行就处理一行,每一个像素点:RGB(应该根据dinfo.output_components设置)
		//一行的像素点数据存储在buffer数组中
		i=0;
		for(x=0;x<dinfo.output_width;x++)  //显示一行数据
		{
			//每一个点的处理
