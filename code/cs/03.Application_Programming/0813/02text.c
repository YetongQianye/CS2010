...
void *producer_f(void *arg)
{
	while(1)
	{
		pthread_mutex_lock(&mutex); //获取mutex指向的线程互斥锁
		buffer++;
		printf("producer_f:buffer = %d\n",buffer);
		if(buffer >= 1000)
		{
			//唤醒等待条件的线程
		}
...
{
	pthread_mutex_t mutex;//定义一个线程互斥锁
	int r =  pthread_mutex_init(&mutex,NULL);   //初始化线程锁
	if(-1 == r)
	{
	...
	pthread_create(&pro_id, NULL,producer_f, NULL);
