...
BiTNode *SingleRotateWithLeft(BiTNode *k)
{
	...
	//更新k.k1的高度
	k->h=MAX(hight_tree(k->rchild),hight_tree(k->lchild))+1;
	k1->h=MAX(hight_tree(k1->rchild),hight_tree(k1->lchild))+1;
	return k1;
}
...
BiTNode *insert_node_to_AVLtree(BiTNode *t,TElemType x)
{
	...
	if(x>t->data) //x的值比根节点大,把x加入到t的右子树上
	{
		...
		//已经加入了结点(x),这棵树的高度肯定改变了
		t->h = MAX(hight_tree(t->rchild),hight_tree(t->lchild))+1;
